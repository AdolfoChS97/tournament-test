...
export class CreatedTournament {
  @ApiProperty({ type: 'object', description: 'Datos del torneo creado', required: true, example: { tournamentId: 'abcdef', accessPrice: 100, rewardsByRanking: { rank: 1, reward: 1000 } } })
  data: CreateTournamentQueryParamsDto & CreateTournamentBodyDto;

  @ApiProperty({ type: 'string', description: 'Mensaje de éxito', required: true, example: 'Tournament created successfully' })
  message: string;
}
